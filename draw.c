...
void scanline_convert( struct matrix *points, int i, screen s, zbuffer zb ) {
    ...
    int top_index = i;
    if (points->m[1][top_index + 2] > points->m[1][top_index])
        top_index++;
    if (points->m[1][top_index + 1] > points->m[1][top_index])
        top_index++;

    int mid_index;
    if (top_index == i) {
        mid_index = points->m[1][i + 1] > points->m[1][i + 2] ? i + 1 : i + 2;
    }
    if (top_index == i + 1) {
        mid_index = points->m[1][i] > points->m[1][i + 2] ? i : i + 2;
    }
    if (top_index == i + 2) {
        mid_index = points->m[1][i] > points->m[1][i + 1] ? i : i + 1;
    }

    int bot_index = i;
    int j;
    for (j = 0; j < 2; j++) {
        if (top_index == bot_index || mid_index == bot_index) {
            bot_index++;
        }
    }

    //printf("top point: %f %f %f\n", points->m[0][top_index], points->m[1][top_index], points->m[2][top_index]);
    //printf("mid point: %f %f %f\n", points->m[0][mid_index], points->m[1][mid_index], points->m[2][top_index]);
    //printf("bot point: %f %f %f\n", points->m[0][bot_index], points->m[1][bot_index], points->m[2][top_index]);

    double x0 = points->m[0][top_index];
    double x1 = points->m[0][top_index];
    double y = (int) points->m[1][top_index];
    double z0 = points->m[2][top_index];
    double z1 = points->m[2][top_index];
    double dx0 = -(points->m[0][bot_index] - x0) / (points->m[1][bot_index] - y);
    double dx1 = -(points->m[0][mid_index] - x1) / (points->m[1][mid_index] - y);
    double dz0 = -(points->m[2][bot_index] - z0) / (points->m[1][bot_index] - y);
    double dz1 = -(points->m[2][mid_index] - z1) / (points->m[1][mid_index] - y);
    ...
    */
    ...
    for (; y > points->m[1][bot_index]; y--) {
        if (y == (int) points->m[1][mid_index]) {
            x1 = points->m[0][mid_index];
            z1 = points->m[2][mid_index];
            dx1 = -(points->m[0][bot_index] - x1) / (points->m[1][bot_index] - y);
            dz1 = -(points->m[2][bot_index] - z1) / (points->m[1][bot_index] - y);
